...
const state = {
  ...
  positionHistory: [], liabilityHistory: [], valueHistory: [], benchmark: 'both',
  trendRange: 'ALL',
...
function seedHistory(base) {
  ...
  for (let i = 365 * 5; i >= 0; i -= 7) {
    const d = new Date(today); d.setDate(today.getDate() - i);
    const drift = 1 + (Math.sin(i / 35) * 0.07 + (365 * 5 - i) * 0.0002);
    const assets = Math.round(base.assets * drift);
    const liabilities = Math.round(base.liabilitiesTotal * (1 - (365 * 5 - i) * 0.00008));
    points.push({ date: d.toISOString().slice(0, 10), assets, liabilities, netWorth: assets - liabilities });
  }
  return points;
}

function filteredHistory() {
  const list = state.valueHistory;
  if (!list.length) return [];
  const end = new Date(list[list.length - 1].date);
  let start = new Date(list[0].date);
  const mode = state.trendRange;
  if (mode === '1D') start = new Date(end.getTime() - 1 * 86400000);
  if (mode === '1W') start = new Date(end.getTime() - 7 * 86400000);
  if (mode === '1Y') start = new Date(end.getTime() - 365 * 86400000);
  if (mode === '5Y') start = new Date(end.getTime() - 365 * 5 * 86400000);
  if (mode === 'YTD') start = new Date(end.getFullYear(), 0, 1);
  if (mode === 'CUSTOM' && state.customRange.start && state.customRange.end) {
    const s = new Date(state.customRange.start); const e = new Date(state.customRange.end);
    return list.filter(v => { const d = new Date(v.date); return d >= s && d <= e; });
  }
  if (mode === 'ALL') return list;
  return list.filter(v => new Date(v.date) >= start && new Date(v.date) <= end);
}
...
function renderCharts(result) {
  ...
  const trend = filteredHistory();
  balanceTrendChart = new Chart(document.getElementById('balanceTrend'), { type: 'line', data: { labels: trend.map(v => v.date), datasets: [{ label: '真實資產', data: trend.map(v => v.assets), borderColor: '#4ba3ff' }, { label: '負債', data: trend.map(v => v.liabilities), borderColor: '#ff7f7f' }, { label: '淨資產', data: trend.map(v => v.netWorth), borderColor: '#39d98a' }] }, options: { responsive: true, interaction: { mode: 'index', intersect: false } } });

  const totalAsset = result.assets || 1; const debtRatio = result.liabilitiesTotal / totalAsset;
...
function bindEvents() {
  const result = calc();
  if (!state.valueHistory.length) state.valueHistory = seedHistory(result);
  const latest = { date: new Date().toISOString().slice(0,10), assets: result.assets, liabilities: result.liabilitiesTotal, netWorth: result.netWorth };
  state.valueHistory[state.valueHistory.length - 1] = latest;
  ...
  document.getElementById('rangeControls').addEventListener('click', e => {
    ...
    document.querySelectorAll('#rangeControls button').forEach(b => b.classList.toggle('active', b === btn));
    rerender();
  });
  document.getElementById('customRangeForm').addEventListener('submit', e => {
    e.preventDefault();
    const fd = new FormData(e.target);
    state.customRange.start = String(fd.get('start')); state.customRange.end = String(fd.get('end'));
    state.trendRange = 'CUSTOM';
    document.querySelectorAll('#rangeControls button').forEach(b => b.classList.remove('active'));
    rerender();
  });

  const positionForm = document.getElementById('positionForm');
  const symbolInput = positionForm.elements.symbol; const marketInput = positionForm.elements.market;
  const clearQuoteCache = () => { delete positionForm.dataset.quoteName; delete positionForm.dataset.quoteSymbol; };
  async function autoFillPrice() { const symbol = String(symbolInput.value || '').trim(); const market = String(marketInput.value || 'US'); clearQuoteCache(); if (!symbol) return; setStatus('正在查詢 Yahoo Finance...'); try { const quote = await fetchQuote(symbol, market); if (!quote.exists || quote.price == null) return setStatus(`查無標的或無報價：${symbol}`, true); positionForm.elements.price.value = Number(quote.price); positionForm.dataset.quoteName = quote.name || symbol.toUpperCase(); positionForm.dataset.quoteSymbol = quote.symbol || symbol.toUpperCase(); setStatus(`已帶入 ${quote.symbol} 現價 ${quote.price}`); } catch (err) { setStatus('現價取得失敗，請稍後再試或手動輸入', true); } }
  symbolInput.addEventListener('blur', autoFillPrice); symbolInput.addEventListener('input', clearQuoteCache); marketInput.addEventListener('change', autoFillPrice); document.getElementById('fetchQuoteBtn').addEventListener('click', autoFillPrice);

  positionForm.addEventListener('submit', e => { e.preventDefault(); const data = new FormData(e.target); const market = String(data.get('market')); const rawSymbol = String(data.get('symbol')).toUpperCase(); const row = { id: uid(), market, symbol: positionForm.dataset.quoteSymbol || rawSymbol, name: positionForm.dataset.quoteName || rawSymbol, shares: Number(data.get('shares')), cost: Number(data.get('cost')), price: Number(data.get('price')), ccy: market === 'US' ? 'USD' : 'TWD' }; state.positions.push(row); state.positionHistory.unshift({ at: now(), action: '新增', symbol: row.symbol, market: row.market, note: `股數 ${row.shares}` }); e.target.reset(); clearQuoteCache(); rerender(); });
  document.getElementById('liabilityForm').addEventListener('submit', e => { e.preventDefault(); const data = new FormData(e.target); const row = { id: uid(), type: String(data.get('type')), balance: Number(data.get('balance')), rate: Number(data.get('rate')), monthlyPayment: Number(data.get('monthlyPayment')), startDate: String(data.get('startDate')), termMonths: Number(data.get('termMonths')), fees: Number(data.get('fees') || 0) }; state.liabilities.push(row); state.liabilityHistory.unshift({ at: now(), action: '新增', type: row.type, note: `餘額 ${format(row.balance)}` }); e.target.reset(); rerender(); });
