...
function bindEvents() {
  ...
  document.getElementById('liabilityForm').addEventListener('submit', e => { e.preventDefault(); const data = new FormData(e.target); const row = { id: uid(), type: String(data.get('type')), balance: Number(data.get('balance')), rate: Number(data.get('rate')), monthlyPayment: Number(data.get('monthlyPayment')) }; state.liabilities.push(row); state.liabilityHistory.unshift({ at: now(), action: '新增', type: row.type, note: `餘額 ${format(row.balance)}` }); e.target.reset(); rerender(); });
