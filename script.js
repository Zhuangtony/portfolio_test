...
const setStatus = (text, isError = false) => {
  ...
  el.textContent = text;
  el.className = isError ? 'negative' : 'positive';
};
...
function calc() {
  ...
  const liabilitiesTotal = state.liabilities.reduce((s, l) => s + l.balance, 0);
  const netWorth = assets - liabilitiesTotal;
  return { enriched, assets, cost, pnl, liabilitiesTotal, netWorth };
}

function renderSummary(result) { /* unchanged */
  const cards = [
    ['總資產', `NT$ ${format(result.assets)}`], ['總負債', `NT$ ${format(result.liabilitiesTotal)}`], ['淨資產', `NT$ ${format(result.netWorth)}`],
    ['未實現損益', `NT$ ${format(result.pnl)}`, result.pnl >= 0], ['投組報酬率', percent(result.cost ? result.pnl / result.cost : 0), result.pnl >= 0],
  ];
...
function renderTables(result) {
  document.querySelector('#positionsTable tbody').innerHTML = result.enriched.map(p => `<tr><td>${p.market}</td><td>${p.symbol}</td><td>${p.name}</td><td>${format(p.shares)}</td><td>${format(p.costTwd)}</td><td>${format(p.valueTwd)}</td><td class="${p.pnl >= 0 ? 'positive' : 'negative'}">${format(p.pnl)}</td><td class="${p.roi >= 0 ? 'positive' : 'negative'}">${percent(p.roi)}</td></tr>`).join('');
  document.querySelector('#liabilitiesTable tbody').innerHTML = state.liabilities.map(l => `<tr><td>${l.type}</td><td>${format(l.balance)}</td><td>${l.rate.toFixed(2)}%</td><td>${format(l.monthlyPayment)}</td></tr>`).join('');
}

function renderCharts(result) {
  if (pieChart) pieChart.destroy(); if (benchmarkChart) benchmarkChart.destroy();
  pieChart = new Chart(document.getElementById('assetPie'), { type: 'pie', data: { labels: result.enriched.map(p => `${p.symbol} (${p.market})`), datasets: [{ data: result.enriched.map(p => p.valueTwd) }] } });
  const datasets = [{ label: '我的投組', data: benchmarkSeries.portfolio, borderColor: '#67b3ff' }];
  if (state.benchmark === 'sp500' || state.benchmark === 'both') datasets.push({ label: 'S&P 500', data: benchmarkSeries.sp500, borderColor: '#39d98a' });
  if (state.benchmark === 'twii' || state.benchmark === 'both') datasets.push({ label: 'TAIEX', data: benchmarkSeries.twii, borderColor: '#ffcb6b' });
  ...
  document.getElementById('allocationList').innerHTML = result.enriched
    .map(p => {
      const ratio = (p.valueTwd / totalAsset) * 100;
      return `<div class="allocation-item"><span>${p.symbol} (${p.market})</span><span>${ratio.toFixed(1)}% / NT$ ${format(p.valueTwd)}</span></div>`;
    })
    .join('');
...
function bindEvents() {
  ...
  document.getElementById('fetchQuoteBtn').addEventListener('click', async () => {
    ...
    const market = String(data.get('market') || 'US');
    if (!symbol) return setStatus('請先輸入股票代號', true);
    ...
    positionForm.elements.name.value = quote.name || symbol;
    positionForm.elements.price.value = Number(quote.price);
    setStatus(`已驗證 ${quote.symbol} (${quote.exchangeName}) 現價 ${quote.price} ${quote.currency}`);
  });

  positionForm.addEventListener('submit', e => {
    e.preventDefault();
    const data = new FormData(e.target); const market = data.get('market');
    state.positions.push({ market, symbol: String(data.get('symbol')).toUpperCase(), name: String(data.get('name')), shares: Number(data.get('shares')), cost: Number(data.get('cost')), price: Number(data.get('price')), ccy: market === 'US' ? 'USD' : 'TWD' });
    e.target.reset(); rerender();
  });

  document.getElementById('liabilityForm').addEventListener('submit', e => {
    e.preventDefault(); const data = new FormData(e.target);
    state.liabilities.push({ type: String(data.get('type')), balance: Number(data.get('balance')), rate: Number(data.get('rate')), monthlyPayment: Number(data.get('monthlyPayment')) });
    e.target.reset(); rerender();
  });
