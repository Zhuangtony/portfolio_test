...
function renderSummary(result) {
  const cards = [
    ...
    ['投組報酬率', percent(result.cost ? result.pnl / result.cost : 0), result.pnl >= 0],
  ];

  document.getElementById('summaryCards').innerHTML = cards.map(([name, value, positive]) => `
    <article class="card"><div class="metric-name">${name}</div><div class="metric-value ${positive === undefined ? '' : positive ? 'positive' : 'negative'}">${value}</div></article>
  `).join('');
}

function renderTables(result) {
  document.querySelector('#positionsTable tbody').innerHTML = result.enriched.map(p => `
    <tr><td>${p.market}</td><td>${p.symbol}</td><td>${p.name}</td><td>${format(p.shares)}</td><td>${format(p.costTwd)}</td><td>${format(p.valueTwd)}</td><td class="${p.pnl >= 0 ? 'positive' : 'negative'}">${format(p.pnl)}</td><td class="${p.roi >= 0 ? 'positive' : 'negative'}">${percent(p.roi)}</td></tr>
  `).join('');

  document.querySelector('#liabilitiesTable tbody').innerHTML = state.liabilities.map(l => `
...
function renderCharts(result) {
  ...
  pieChart = new Chart(document.getElementById('assetPie'), {
    type: 'pie',
    data: { labels: result.enriched.map(p => `${p.symbol} (${p.market})`), datasets: [{ data: result.enriched.map(p => p.valueTwd) }] },
  });
  ...
  benchmarkChart = new Chart(document.getElementById('benchmarkLine'), {
    type: 'line',
    data: { labels: benchmarkSeries.labels, datasets },
...
function bindEvents() {
  ...
  if (savedTheme === 'light') {
    ...
    themeToggle.textContent = '切換背景：淺色';
  }
  ...
  themeToggle.addEventListener('click', () => {
    ...
    themeToggle.textContent = `切換背景：${isLight ? '淺色' : '深色'}`;
  });
