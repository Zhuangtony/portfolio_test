...
function renderCharts(result){if(pieChart)pieChart.destroy();if(benchmarkChart)benchmarkChart.destroy();if(balanceTrendChart)balanceTrendChart.destroy();const pieLabels=result.grouped.map(p=>`${p.symbol} (${p.market})`).concat('負債');const pieValues=result.grouped.map(p=>p.valueTwd).concat(result.liabilitiesTotal);pieChart=new Chart(document.getElementById('assetPie'),{type:'pie',data:{labels:pieLabels,datasets:[{data:pieValues}]},options:{maintainAspectRatio:false,plugins:{legend:{position:'bottom'}}}});const datasets=[{label:'我的投組',data:benchmarkSeries.portfolio,borderColor:'#67b3ff'}];if(state.benchmark!=='twii')datasets.push({label:'S&P 500',data:benchmarkSeries.sp500,borderColor:'#39d98a'});if(state.benchmark!=='sp500')datasets.push({label:'TAIEX',data:benchmarkSeries.twii,borderColor:'#ffcb6b'});benchmarkChart=new Chart(document.getElementById('benchmarkLine'),{type:'line',data:{labels:benchmarkSeries.labels,datasets},options:{responsive:true,interaction:{mode:'index',intersect:false}}});const trend=filteredHistory();balanceTrendChart=new Chart(document.getElementById('balanceTrend'),{type:'line',data:{labels:trend.map(v=>v.date),datasets:[{label:'真實資產',data:trend.map(v=>v.assets),borderColor:'#4ba3ff'},{label:'負債',data:trend.map(v=>v.liabilities),borderColor:'#ff7f7f'},{label:'淨資產',data:trend.map(v=>v.netWorth),borderColor:'#39d98a'}]},options:{responsive:true,interaction:{mode:'index',intersect:false}}});const totalAsset=result.assets||1;const debtRatio=result.liabilitiesTotal/totalAsset;document.getElementById('allocationList').innerHTML=result.grouped.map(p=>`<div class="allocation-item"><span>${p.symbol} (${p.market})</span><span>${((p.valueTwd/totalAsset)*100).toFixed(1)}% / NT$ ${format(p.valueTwd)}</span></div>`).join('')+`<div class="allocation-item debt"><span>負債</span><span>${(debtRatio*100).toFixed(1)}% / NT$ ${format(result.liabilitiesTotal)}</span></div>`;}
function rerender(){const result=calc();renderSummary(result);renderTables(result);renderCharts(result);}
function bindEvents(){const result=calc();if(!state.valueHistory.length)state.valueHistory=seedHistory(result);state.valueHistory[state.valueHistory.length-1]={date:new Date().toISOString().slice(0,10),assets:result.assets,liabilities:result.liabilitiesTotal,netWorth:result.netWorth};const body=document.body,themeToggle=document.getElementById('themeToggle'),savedTheme=localStorage.getItem('theme')||'dark';if(savedTheme==='light'){body.classList.add('light');themeToggle.textContent='切換背景：淺色';}themeToggle.addEventListener('click',()=>{body.classList.toggle('light');const isLight=body.classList.contains('light');localStorage.setItem('theme',isLight?'light':'dark');themeToggle.textContent=`切換背景：${isLight?'淺色':'深色'}`;});
  ...
  const positionForm=document.getElementById('positionForm'),symbolInput=positionForm.elements.symbol,marketInput=positionForm.elements.market;const clearQuoteCache=()=>{delete positionForm.dataset.quoteName;delete positionForm.dataset.quoteSymbol;};
  ...
  positionForm.addEventListener('submit',e=>{e.preventDefault();const data=new FormData(e.target);const market=String(data.get('market'));const rawSymbol=String(data.get('symbol')).toUpperCase();const row={id:uid(),market,symbol:positionForm.dataset.quoteSymbol||rawSymbol,name:positionForm.dataset.quoteName||rawSymbol,shares:Number(data.get('shares')),cost:Number(data.get('cost')),price:Number(data.get('price')),ccy:market==='US'?'USD':'TWD',buyDate:String(data.get('buyDate'))};state.positions.push(row);state.positionHistory.unshift({at:now(),action:'新增',symbol:row.symbol,market:row.market,note:`買入日 ${row.buyDate}`});e.target.reset();clearQuoteCache();rerender();});
